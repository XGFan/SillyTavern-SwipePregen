...
Six details are load-bearing and each was a bug before it was understood:
...
- The chat's scroll position is pinned for as long as the mask is up (`lockChatScroll`). ST scrolls the chat to the bottom of the message it is writing into (`expandNewMessage` → `getMessageBottomHeight`, script.js:10256 and 10261), and under a 遮挡层 that message is the one taken out of the flow, with its `.mes_text` already emptied to `...` — so the number it computes describes a collapsed stand-in and the chat jumps up by roughly the height of the reply on screen, for the whole generation and again on every generation of a 排队. `addOneMessage`'s own `scrollChatToBottom` is the same story pointing the other way, for a reader who scrolled up mid-run. The pin is a `scrollTop` setter override on `#chat`: only programmatic writes go through the property, so the wheel, the scrollbar, touch and the keyboard still belong to the reader. Measured on a 排队 of three: −834px at each generation's start before, 0 after.
